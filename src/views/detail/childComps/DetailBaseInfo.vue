<template>
  <div class="base-info" v-if="Object.keys(goods).length !== 0">
    ...
    <div class="info-price">
      ...
      <span :style="{backgroundColor: goods.discountBgColor}" class="discount" v-if="goods.discount" >{{goods.discount}}</span>
    </div>
    <div class="info-other">
      <span>{{goods.columns[0]}}</span>
      <span class="info-other-mid">{{goods.columns[1]}}</span>
      <span>{{goods.services[goods.services.length - 1].name}}</span>
    </div>
    <div class="info-service">
      <div class="info-service-item" v-for="index in goods.services.length - 1" :key="index">
        <span v-if="goods.services[index - 1].icon">
